...
namespace VC.WebApi.Infrastructure.EFCore.ModelBuilderExtensions
{
    ...
    {
        public static void ApplyGuidIdValueConverters(this ModelBuilder modelBuilder)
        {
            // List of all EF Core objects and properties that are mapped to the db
            foreach (var entityType in modelBuilder.Model.GetEntityTypes())
            {
                var properties = entityType.ClrType.GetProperties()
                    .Where(p => typeof(GuidId).IsAssignableFrom(p.PropertyType));

                foreach (var property in properties)
                {

                    var converterType = typeof(GuidIdValueConverter<>).MakeGenericType(property.PropertyType);

                    var converter = Activator.CreateInstance(converterType);
                    if (converter is null)
                    {
                        throw new Exception("Error creating GuidIdValueConverter Object: " + entityType.ClrType.ToString() + " Property: " + property.ToString());
                    }
                ...
                foreach (var ownedNavigation in ownedTypes)
                {
                    ...
                    {
                        var converterType = typeof(GuidIdValueConverter<>).MakeGenericType(ownedProperty.PropertyType);
                        var converter = Activator.CreateInstance(converterType);
                        if (converter is null)
                        {
                            throw new Exception("Error creating GuidIdValueConverter Object: " + ownedEntityType.ToString() + " Property: " + ownedProperty.ToString());
                        }
